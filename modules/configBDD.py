...
from modules.sincGithub import sincronizar_bd_con_github, eliminar_bd_en_github, sincronizar_config_con_github
...
def guardar_bd_activa(ruta_bd):
    try:
        with open(ARCHIVO_CONFIG_APP, 'w', encoding = 'utf-8') as f:
            if ruta_bd:
                f.write(os.path.basename(ruta_bd))
            else:
                f.write('')
        sincronizar_config_con_github()
    except Exception:
        pass
    ...
    try:
        if os.path.exists(ARCHIVO_CONFIG_APP):
            with open(ARCHIVO_CONFIG_APP, 'r', encoding = 'utf-8') as f:
                nombre_bd = f.read().strip()
                if nombre_bd:
                    ruta = os.path.join(directorioBase, nombre_bd)
        ...
        with st.expander('Haz clic aquí para eliminar una base de datos existente'):
            ...
            if st.button('Eliminar Base de Datos'):
                try:
                    ...
                    eliminar_bd_en_github(bdAEliminar)
